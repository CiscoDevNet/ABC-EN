...
class TestInterfaces(aetest.Testcase):
    ...
    def test_interface_state(self, interface_name, steps):
        ...
        logger.error("Current interface:\n%s", current_interface)
        logger.error("Current interface:\n%s", current_interface.shutdown)
        logger.error("Current interface:\n%s", current_interface.enabled)
        ...
        with steps.start("Admin state (shutdown)", continue_=True) as step:
            ...
            desired_state = current_interface.enabled or UNSPECIFIED_SHUTDOWN_STATE

            # Use "verify_interface_state_admin_down" as it reflects the same
            # boolean state as interface.shutdown
            configured_state = self.device.parse(f"show interfaces {interface_name}") # ,
            #     interface=interface_name, max_time=1
            # )
            logger.error("Configured state:\n%s", configured_state[interface_name]["enabled"])
            try:
                if desired_state:
                    assert configured_state[interface_name]["enabled"]
                else:
                    assert not configured_state[interface_name]["enabled"]
            except AssertionError:
                step.failed(
                    f"Desired interface enable state: {desired_state}, "
                    f"configured is {configured_state[interface_name]['enabled']}"
                )
            else:
        ...
        with steps.start("Interface IPv6 address", continue_=True) as step:
            ...
            try:
                ...
                logger.error("Testing that interface addr: %s matches configured: %s", str(current_interface.ipv6).upper(), str(configured_address).upper())
                ...
                step.failed(
                    f"Expecting {current_interface.ipv6}, configured is {configured_address}"
                )
            else:
